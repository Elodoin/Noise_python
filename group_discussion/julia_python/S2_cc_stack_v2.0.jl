...
# this is slightly different from the python output (by 1 sample)  by Zhitu Ma
...
const nx2=72900
const ny=47
const FFTDIR = "/Users/chengxin/Documents/Harvard/Julia/julia_noise_shared/data_test/FFT/"
const CCFDIR = "/Users/chengxin/Documents/Harvard/Julia/julia_noise_shared/data_test/CCF/julia/"
const maxlag=800
...
#            for jj=1:length(allfiles)  #for easier comparison with python output 
            for jj=ii+1:length(allfiles)  #you might want to use this for real run
                ...
                for name_r in filter(x->x[end-3:end]=="real",names(hdf5_r))
                    name_r=name_r[1:end-5]
                    tmp=split(name_r,"_")
                    nsta_r=tmp[3]
                    comp_r=tmp[4]
...
#                    println("output ",join((nsta_s,comp_s,nsta_r,comp_r),"_"))
                    t2=time()
