...
for ii in range(rank,splits+size-extra,size):
    ...
    if ii<splits:
        ...
        with pyasdf.ASDFDataSet(dayfile,mpi=False,mode='r') as ds:
            ...
            #-------make a crutial estimate on the memory needed to load the daily CC files--------
            ...
            for ii in range(len(data_types)):
                ...
                for jj in range(len(tpaths)):
                    ...
                    c3_win[k][:] = noise_module.get_coda_window(cc_dist[k],vmin,maxlag,dt,wcoda)
                    
                    if flag:
                        print('station pair %s %s is %7.1f km apart' % (data_type,tpath,cc_dist[k]))
                    k+=1

            np.save('cc_array.dat',cc_array)
            if k != npairs:
            ...
            for ii in range(npairs):
                ...
                #---use index in all_list to identify the order of source or receiver-----
                ...
                for indx in range(len(all_list)):
                    ...
                    ccp,ccn=noise_module.C3_process(S1_data,S2_data,Nfft,c3_win[k1],c3_win[k2])
                    cc_P+=ccp
                    cc_N+=ccn
                    tpairs+=1
