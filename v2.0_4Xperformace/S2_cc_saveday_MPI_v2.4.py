...
for ii in range(rank,splits+size-extra,size):
    ...
    if ii<splits:
        ...
        cc_std   = np.zeros((nsta*ncomp,Nseg),dtype=np.float32)
        ...
        for ifile in range(len(sfiles)):
            ...
            if flag:
                print('reading station %s' % tfile.split('/')[-1])
                ...
                if len(data_types) < ncomp:
                    ...
                    for icomp in data_types:
                        ...
                        if iday in tpaths:
                            ...
                            data  = np.zeros((Nseg,Nfft//2),dtype=np.complex64)
                            data  = ds.auxiliary_data[icomp][iday].data[:]
                            cc_array[indx][:]= data.reshape(data.size)
                            cc_std[indx][:]  = ds.auxiliary_data[icomp][iday].parameters['std']
                else:
                    ...
                    for ii in range(len(data_types)):
                        ...
                        if iday in tpaths:
                            indx = ifile*ncomp+ii
                            data  = np.zeros((Nseg,Nfft//2),dtype=np.complex64)
                            data  = ds.auxiliary_data[data_types[ii]][iday].data[:]
                            cc_array[indx][:]= data.reshape(data.size)
                            cc_std[indx][:]  = ds.auxiliary_data[data_types[ii]][iday].parameters['std']

        tt0 = time.time()
        print('loading all FFT takes %6.4fs' % (tt0-tt))
        ...
        tt1 = time.time()
        print('it takes %6.4fs to process %d hours of data in step 2' % (tt1-tt0, Nseg))
