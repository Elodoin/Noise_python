    ...
    if ista<splits:
        ...
        if input_asdf:    
            ...
            for jj in range (len(tdir)):
                ...
                #-------no fancy algorithm for cross-correlations--------
                else:
                    
                    #--------do normalization on three components simutaneously-------
                    if len(tfiles)!=3:
                        print('Continue! no 3 components for station %s' % station)
                    
                    sacfile = os.path.basename(tfiles[0])

                    #------use 3 variables here to avoid the complication from stream to stream arrays-----
                    try:
                        source1 = obspy.read(tfiles[0])
                        source2 = obspy.read(tfiles[1])
                        source3 = obspy.read(tfiles[2])
                    except Exception as inst:
                        print(inst)
                        continue

                    comp1 = source1[0].stats.channel
                    comp2 = source2[0].stats.channel
                    comp3 = source3[0].stats.channel
                        
                    if flag:
                        print("working on sacfile %s" % sacfile)

                    #---------make three inventories now---------
                    inv1=noise_module.stats2inv(source1[0].stats)
                    inv2=noise_module.stats2inv(source2[0].stats)
                    inv3=noise_module.stats2inv(source3[0].stats)
                    
                    if prepro:
                        t0=time.time()
                        source1 = noise_module.preprocess_raw(source1,downsamp_freq,checkt,pre_filt,resp,resp_dir)
                        source2 = noise_module.preprocess_raw(source2,downsamp_freq,checkt,pre_filt,resp,resp_dir)
                        source3 = noise_module.preprocess_raw(source3,downsamp_freq,checkt,pre_filt,resp,resp_dir)
                        if len(source1)==0 or len(source2)==0 or len(source3)==0:
                            continue
                        t1=time.time()
                    ...
                    #----------variables to define days with earthquakes----------
                    all_madS1 = noise_module.mad(source1[0].data)
                    all_stdS1 = np.std(source1[0].data)
                    all_madS2 = noise_module.mad(source2[0].data)
                    all_stdS2 = np.std(source2[0].data)
                    all_madS3 = noise_module.mad(source3[0].data)
                    all_stdS3 = np.std(source3[0].data)
                    if all_madS1==0 or all_stdS1==0 or all_madS2==0 or all_stdS2==0 or all_madS3==0 or all_stdS3==0 :
                        print("continue! madS or stdS equeals to 0 for %s" % sacfile)
                        continue
                    ...
                    nonzeroS1 = []
                    nptsS1 = []
                    source_slice1 = obspy.Stream()

                    #--------break a continous recording into pieces----------
                    t0=time.time()
                    for ii,win in enumerate(source1[0].slide(window_length=cc_len, step=step)):
                        win.detrend(type="constant")
                        win.detrend(type="linear")
                        trace_madS1.append(np.max(np.abs(win.data))/all_madS1)
                        trace_stdS1.append(np.max(np.abs(win.data))/all_stdS1)
                        nonzeroS1.append(np.count_nonzero(win.data)/win.stats.npts)
                        nptsS1.append(win.stats.npts)
                        win.taper(max_percentage=0.05,max_length=20)
                        source_slice1.append(win)
                    del source1
                    ...
                    nonzeroS2 = []
                    nptsS2 = []
                    source_slice2 = obspy.Stream()
                    for ii,win in enumerate(source2[0].slide(window_length=cc_len, step=step)):
                        win.detrend(type="constant")
                        win.detrend(type="linear")
                        trace_madS2.append(np.max(np.abs(win.data))/all_madS2)
                        trace_stdS2.append(np.max(np.abs(win.data))/all_stdS2)
                        nonzeroS2.append(np.count_nonzero(win.data)/win.stats.npts)
                        nptsS2.append(win.stats.npts)
                        win.taper(max_percentage=0.05,max_length=20)
                        source_slice2.append(win)
                    del source2
                    ...
                    nonzeroS3 = []
                    nptsS3 = []
                    source_slice3 = obspy.Stream()
                    for ii,win in enumerate(source3[0].slide(window_length=cc_len, step=step)):
                        win.detrend(type="constant")
                        win.detrend(type="linear")
                        trace_madS3.append(np.max(np.abs(win.data))/all_madS3)
                        trace_stdS3.append(np.max(np.abs(win.data))/all_stdS3)
                        nonzeroS3.append(np.count_nonzero(win.data)/win.stats.npts)
                        nptsS3.append(win.stats.npts)
                        win.taper(max_percentage=0.05,max_length=20)
                        source_slice3.append(win)
                    del source3
                    ...
                    if len(source_slice1) == 0 or len(source_slice2)==0 or len(source_slice3):
                        print("No traces for %s " % tfiles[0])
                        continue

                    source_params1= np.vstack([trace_madS1,trace_stdS1,nonzeroS1]).T
                    source_params2= np.vstack([trace_madS2,trace_stdS2,nonzeroS2]).T
                    source_params3= np.vstack([trace_madS3,trace_stdS3,nonzeroS3]).T

                    #---------seems un-necesary for data already pre-processed with same length (zero-padding)-------
                    N = len(source_slice1)
                    NtS = np.max(nptsS1)
                    dataS1 = np.zeros(shape=(N,NtS),dtype=np.float32)
                    for ii,trace in enumerate(source_slice1):
                        dataS1[ii,0:nptsS[ii]] = trace.data
                        if ii==0:
                            dataS_stats1=trace.stats

                    dataS2 = np.zeros(shape=(N,NtS),dtype=np.float32)
                    for ii,trace in enumerate(source_slice2):
                        dataS2[ii,0:nptsS[ii]] = trace.data
                        if ii==0:
                            dataS_stats2=trace.stats

                    dataS3 = np.zeros(shape=(N,NtS),dtype=np.float32)
                    for ii,trace in enumerate(source_slice3):
                        dataS3[ii,0:nptsS[ii]] = trace.data
                        if ii==0:
                            dataS_stats3=trace.stats

                    #---2 parameters----
                    axis = dataS1.ndim-1
                    Nfft = int(next_fast_len(int(dataS1.shape[axis])))

                    #------time domain normalization-----
                    t0=time.time()
                    if norm_type == 'running_mean': 
                        #white1,white2,white3 = noise_module.moving_ave_3comp(dataS1,dataS2,dataS3,smooth_N)
                        white = dataS1/noise_module.moving_ave(dataS1,smooth_N)
                    source_white1 = scipy.fftpack.fft(white1, Nfft, axis=axis)
                    source_white2 = scipy.fftpack.fft(white2, Nfft, axis=axis)
                    source_white3 = scipy.fftpack.fft(white3, Nfft, axis=axis)
                    t1=time.time()
                    if flag:
                        print("temporal normalization takes %f s"%(t1-t0))
                    ...
                    t0=time.time()
                    #source_white1,source_white2,source_white3 = noise_module.whiten_3comp(source_white1,source_white2,source_white3,dt,freqmin,freqmax)
                    source_white = noise_module.whiten_smooth(white,dt,freqmin,freqmax,smooth_N)
                    t1=time.time()
                    if flag:
                        print("spectral whitening takes %f s"%(t1-t0))
