...
#rootpath = '/n/holylfs/EXTERNAL_REPOS/DENOLLE/DATA_CJIANG/KANTO'
FFTDIR = os.path.join(rootpath,'FFT')
CCFDIR = os.path.join(rootpath,'CCF1')
...
for ii in range(rank,splits+size-extra,size):
    ...
    if ii<splits:
        ...
        for iseg in range(num_seg):
            ...
            if memory_size > 4:
                raise MemoryError('Memory exceeds 4 GB! No enough memory to load them all once!')
                    ...
                    if len(data_types) < ncomp:
                        ...
                        for icomp in data_types:
                            ...
                            if iday in tpaths:
                                ...
                                #-----check bounds-----
                                if indx > ntrace:
                                    raise ValueError('indx out of bounds')
        ...
        tt1 = time.time()
        print('it takes %6.4fs to process %d hours of day %s in step 2' % (tt1-tt0,nhours,iday))
